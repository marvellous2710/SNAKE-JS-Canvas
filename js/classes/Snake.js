//ce qui défini la taille du snake
class Snake {
    constructor() {
        this.x = 0;
        this.y = 0;
    ...
    moveHead() {//direction du snake
        const head = this.blocks[0];
        head.oldX = head.x;//on sauvegarde les positions avant de les bouger
        head.oldY = head.y;
    ...
    calculateNewBlockPosition() {
        let {x, y} = this.blocks[this.blocks.length -1]; //c'est comme si on faisait let x = head.x   et   let y = head.y
        switch (currentDirection) {
            case 'left':
                x += 1;
                break;
    ...
    update() {
        ...
        for (const [index, block] of this.blocks.entries()) {
            if (index > 0) {
                const {oldX, oldY} = this.blocks[index - 1];//pour faire suivre les blocks quand on en mange un autre
